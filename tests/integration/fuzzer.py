...
class _Fuzzer_Worker:
    ...
    def run_with(self, iteration, mimic):
        ...
        try:
            ...
            logger.debug("Cluster creation time: {}".format((end-start).total_seconds()))
        ...
        try:
            ...
            logger.debug("Iteration running time: {}".format(diff))
            try:
                cluster.destroy()
            except:
                logs = get_logs(cluster)
...
class Fuzzer:
    ...
    def run_part(self):
        ...
        with self.sync.done_cond:
            self.sync.done_cond.wait_for(self.sync.is_done)
    ...
    def run(self):
        ...
        self.sync.reset_offset(0)
        self.sync.target_iterations = self.config.iterations
        self.run_part()
